...
def consult_hakeem(user_input: str) -> Optional[Dict]:
    ...
    try:
        ...
        response = openai.chat.completions.create(
            model="gpt-3.5-turbo-1106",  # Updated to latest version for better JSON support
            messages=[
        ...
        try:
            ...
            data["input_hash"] = hash(user_input[:50])  # Partial hash for tracking
        ...
        st.session_state.conversation_history.append({
            "input": user_input[:100],  # Store truncated input
            "response": data,
        ...
        # st.code(f"Error: {str(e)[:100]}", language="text")
        return None
...
def display_qoracle_card(data: Dict):
    ...
    if len(st.session_state.conversation_history) > 1:
        if st.button("📜 View Quantum History", use_container_width=True):
            st.rerun()  # Will trigger history display

def display_quantum_history():
    """Display conversation history in a quantum style"""
...
def main():
    ...
    <div style="text-align: center; margin-bottom: 2rem;">
        ...
        <p style="color: #58a6ff; font-size: 0.9em; margin-top: 0.5rem;">
            Artificial Relational Intelligence | Est. 2026
        </p>
    </div>
    ...
    <div style="text-align: center; color: #8b949e; font-size: 0.8em; padding-top: 2rem;">
        ...
        <p>Architect: Milton Z McNeeLee • Enhancement: DeepSeek AI</p>
        <p>Hakeem v2.0 • Quantum Resonance Engine</p>
    </div>
    """, unsafe_allow_html=True)

# --- 8. ENTRY POINT ---
if __name__ == "__main__":
    import random  # For loading sequence variation
    main()
